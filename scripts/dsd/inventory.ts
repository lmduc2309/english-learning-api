...
async function main(): Promise<void> {
  ...
  try {
    const existing: ExistingEntry[] = (
      await ds.query(
        `SELECT headword_normalized AS "headwordNormalized", headword,
    ...
    await ds.transaction(async (manager) => {
      for (const row of plan.toInsert) {
        const { headword, headwordNormalized } = normalizeHeadword(row.headword);
        // UUID is generated by the database. No legacy ID is matched or copied.
        const [entry] = await manager.query(
          `INSERT INTO dsd_entries
             (headword, headword_normalized, dsd_priority, dsd_band, inventory_evidence_id, status)
           VALUES ($1,$2,$3,NULLIF($4,''),$5,'draft') RETURNING id`,
          [headword, headwordNormalized, Number(row.dsd_priority), row.dsd_band ?? '', row.inventory_evidence_id],
        );
        await manager.query(
      ...
      for (const row of plan.toUpdate) {
        ...
        const [entry] = await manager.query(
          `UPDATE dsd_entries
            ...
            WHERE headword_normalized=$4 RETURNING id`,
          [headword, Number(row.dsd_priority), row.dsd_band ?? '', headwordNormalized],
        );
        await manager.query(
          `INSERT INTO dsd_provenance_events
             (entity_kind, entity_id, event_type, actor, evidence_id)
