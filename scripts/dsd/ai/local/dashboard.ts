...
const STAGES = ['inventory', 'inventory_critic', 'english', 'critic', 'translate'] as const;
...
function render(d){let w=d.current;if(!w){app.innerHTML='<div class="panel">No run found.</div>';return}let ss=w.stages,done=Object.values(ss).reduce((n,s)=>n+s.completed,0),fail=Object.values(ss).reduce((n,s)=>n+s.failed,0),tok=Object.values(ss).reduce((n,s)=>n+s.tokens,0);let names=['inventory','inventory_critic','english','critic','translate'];let flow=names.map((n,i)=>stage(n,ss[n],w.active)+(i<names.length-1?'<div class="arrow">→</div>':'')).join('');flow+='<div class="stage '+(w.active==='packaged'?'active':'')+'"><div class="name">package</div><div class="num">'+(w.package_ready?'READY':'WAIT')+'</div><div class="muted">draft only</div></div>';
